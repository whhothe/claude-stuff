...
3. **The script** — the 29-minute Loom answer *(the deliverable)*
...
*This is the deliverable. `[SLIDE: x.png]` cues the visual listed in Part 4. 29 minutes spoken
at 150 wpm, 31 at 140. Regenerated verbatim from `presentation-script.md`.*
...
**do one timed read-through before you record** and cut the "thing I'd want to test" section
first if you're over.* *`[SLIDE: x.png]` cues the visual. Every figure is checked — see
...
I'm [YOUR NAME]. I went through all 330 videos, the month-by-month numbers for every one, the
competitive set and the premise doc.

Two things first. Several times in here I'm going to show you analysis that reversed a
conclusion I'd already written down. I've left those in, because you say a retention curve
should be able to prove you wrong, and I'd rather show you that happening than tell you I'd
accept it.

And the brief says the format is aging and you already know it. I agree — I won't spend your
time arguing otherwise. But I don't think it's aging the way it looks like it is.

---

## QUESTION 1 — WHERE ARE WE FAILING

### Pass one: the brand — 7 min
...
The early months were both low-cadence and early — cadence was riding along with time. I was
about to recommend cutting output by forty percent, and it would have cost you money: in 2026,
higher-cadence months produced **2.05 million** total views against **1.22 million**. There's a
real finding inside that and it's sharper than the one I started with. I'll get to it.

#### What it actually is — 1.75 min
...
Now look at the catalogue. The early topics had it built in — *Footage Caught on Camera* at
**4.03 million**, *CCTV* at **3.17 million**, *Internet Mysteries* at **2.2 million**. Every one
is something a person has already half-wondered about.
...
curiosity about fishing. No itch, so nothing got scratched.
...
#### Why the filter stopped being applied — 1 min
...
#### The rest of it — 2.5 min

Four more, quickly, all real but all secondary to that.

*[SLIDE: sequel-decay.png]*

**Sequels.** Seven multi-volume series, declines of 50 to 96 percent from best volume to latest,
averaging **minus 85**. One exception, and it proves the point — *Things on Asian Internet* went
**up** 94 percent between volumes, because the topic was actually new. The brief makes "which
series earn a next volume" the strategist's call. That call isn't being made.

*[SLIDE: the-eight-video-cliff.png]*

**Eight uploads.** Here's the real finding from the volume analysis. Six uploads a month: 2.07
million total views. Seven: **3.01 million**, the best tier there is. Eight: **1.09 million**.
Per-video at seven is 429,000; at eight it's 136,000. Seven works, eight breaks — and your
premise doc says each channel uploads about twice a week, which is eight a month. **Your stated
cadence is precisely the tier where the numbers collapse.**

*[SLIDE: convergence.png]*

**Convergence.** 494,000 subscribers, 274,000, 123,000 — and average views of 138, 127 and 110
thousand. Four times the audience spread, one identical floor. Three channels can't land on one
...
one brand. The nearest competitor in the set, GOHA, asks for **1.8**. Chilling Scares asks
**1.1** — so you are asking **fifteen times** what Chilling Scares does and nine times what
GOHA does. And the videos in this niche that clear a million views cluster around **28 minutes**
while AD averages **59**.
...
**So why is its baseline falling?** Because the three-test filter never had to bite here. On AO
a bad topic is visibly off-lane and someone notices. On AD almost anything is technically
in-territory, so *Biblical Horrors* and *Fishing Encounters* passed a greenlight that had no
grounds to refuse them. Widest permission, weakest filter, thinnest topics, longest runtime —
in that order, and that's the whole 93 percent.
...
**So why is its baseline falling?** This is the cleanest causal story in the network. **AD2 is
structurally downstream.** It runs Volume 3, 4 and 5 of topics AD1 already spent at Volume 1 and
2 — and network-wide sequel decay is **minus 85 percent**. AD2 doesn't have a decline curve of
its own; it imports AD1's, one volume later and worse. Peak 601,000, now 133,000, **minus 78** —
and the two series driving that are Theme Park at minus 92 and Live TV at minus 87, both
inherited. The repositioning is the right fix and it's half done. The other half is still
someone else's exhausted topics.
...
**Five. Change what a segment ends on.** This is the one that matters, and I'll spend Q2 on it
rather than describing it twice — so here it is in one sentence. Right now a segment ends on the
footage and the toll, so severity is the only thing stacking, which is why the videos need fifty
clips to feel deep. Make every segment end on **the document that already existed** instead —
the levels still escalate on harm, but the depth stops coming from volume and starts coming from
the record. All three of my pitches are that same change applied three times.

#### And the thing I'd want to test, once I'd earned it — 1 min

*[SLIDE: next-format.png]*

You said the strategist has to research the format that comes next, so I'll tell you where I'd
look — but I want to be straight about standing. I've never shipped a video on your channels. I
have your public numbers and nothing else. So this is a hypothesis I'd test cheaply, not a
recommendation for day one.

**One case. Seven levels. Twenty-five to thirty-five minutes.** Same descent, but depth means
further into one incident rather than more incidents. Level 1 is what the public knows, Level 7
is what's in the record nobody has read.

Why I'd want to find out: it removes the fifty-clip constraint entirely, and it lands near the
28-minute mark where this niche's outliers cluster. Why I wouldn't bet the slate on it: if the
single case doesn't hold, nothing else carries the video, and I can't price that from outside.

One video, one channel, one month, against that channel's own baseline.
...
### The change: every segment ends on the paperwork — 2.5 min

*[SLIDE: the-document-axis.png]*

Right now a level is a severity tier. Level 1 is bad, Level 7 is worse. That's why the videos
feel like lists, and why they need fifty clips — severity is the only thing stacking.

The change: **every segment ends on the document.** Not the footage, not the death toll — the
piece of paper that already existed. The inspection that was failed. The report that was filed.
The check that was run and ignored.

I got this wrong once and had to back it out, so let me be precise. My first version made the
*level* mean the document — L1 nobody knew, L7 they knew and signed it off anyway. It reads
beautifully and it fails arithmetic. Your spec needs five to seven cases per level, and "they
knew and did it anyway" is the rarest state there is. You can find one Mottarone. You cannot find
six at the bottom of every topic. So the levels still escalate on harm, which is abundant. The
document is the **signature of every case**, not the definition of the level.

What it buys you. The surface can be completely familiar, because the depth isn't the footage,
it's the report — and inquiry findings and inspection histories genuinely aren't on YouTube.
That matters, because topic novelty is finished: I tested Kitchen Nightmares, child stars, India
and Telegram, and every one already has a video with millions of views on it. It makes the moral
frame structural rather than a line at the end. It escalates harder than a body count. It's
defensible, because pulling inspection histories at volume takes a team a solo competitor doesn't
have. And "the document we couldn't put on YouTube" beats "level 8" as a paid layer.
...
AD's disaster lane has done explosion, roller coaster, mountain, outdoor, water park, Disney park
and incidents at sea. It has never done the thing built by people that fell down while those
people were inside. It's also the most-filmed disaster category there is, which matters because
...
clamp had been fitted to hold it open. The service manager admitted that had become routine
practice to avoid downtime. **14 dead.**
...
**What I'd change.** Stop letting the machine be the subject. Every existing video here is "look
...
**AO gets a different answer, and I want to say why.**
...
with murdering his ex-wife — he held a **2007 Canadian conviction for assaulting a former
girlfriend**. VH1 pulled the series mid-run.

**Hook.** A casting tape. Someone bright and likeable, saying why they'd be great on the show.
...
The short version. You're not failing because the format got old. You're failing because the
runtime requirement quietly took over topic selection, and the filter you already wrote down
stopped being applied. Fix the filter, cap at seven, kill the volume numbers — and stop trying
to win on topic novelty, because that game is over. Win on depth. Make every segment end on the
document.

Four things in this changed while I was making it: volume turned out not to be the cause once I
controlled for time, the health-inspection idea died when I checked whether the records still
existed, the elevator pitch had to widen once I looked at where the footage actually is, and the
document idea itself had to change shape because the first version failed arithmetic.

I've left all four in rather than presenting the tidy version. You wrote that a retention curve
should be able to prove someone wrong. It's easy to agree with that in an interview.
